...
public class AggregateController extends BaseController {
    ...
    @RequestMapping(value = "/external_id/{external_id}", method = RequestMethod.GET,
            produces = MediaType.APPLICATION_JSON_VALUE)
    @ResponseBody
    @Transactional(readOnly = true)
    public Aggregate getAggregate(@PathVariable("external_id") String externalId) {
        Aggregate result = this.aggregateService.getAggregateByExternalId(externalId);
        if (result == null) {
